...
<section class="max-w-3xl rounded-lg border border-slate-200 bg-white shadow-sm">
    ...
    <form method="POST" action="{{ route('super-admin.employers.update', $employer) }}" class="space-y-5 px-5 py-5">
        ...
        <div>
            ...
            <input id="name" type="text" name="name" value="{{ old('name', $employer->name) }}" class="mt-1 block w-full rounded-lg text-sm shadow-sm @error('name') border-rose-300 focus:border-rose-500 focus:ring-rose-500 @else border-slate-300 focus:border-emerald-500 focus:ring-emerald-500 @enderror" required>
            @error('name') <p class="mt-1 text-sm text-rose-600">{{ $message }}</p> @enderror
        </div>

        <div>
            <label for="email" class="block text-sm font-semibold text-slate-700">Email address</label>
            <input id="email" type="email" name="email" value="{{ old('email', $employer->email) }}" class="mt-1 block w-full rounded-lg text-sm shadow-sm @error('email') border-rose-300 focus:border-rose-500 focus:ring-rose-500 @else border-slate-300 focus:border-emerald-500 focus:ring-emerald-500 @enderror" required>
